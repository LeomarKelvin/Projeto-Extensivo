// Adicionei o parâmetro largura (padrão 80)
export const imprimirComanda = (pedido: any, lojaNome: string, larguraMm: number = 80) => {
  ...
  // Ajusta tamanho da fonte baseado na largura do papel
  const fontSize = larguraMm === 58 ? '10px' : '12px';
  const headerSize = larguraMm === 58 ? '12px' : '16px';
    ...
    <html>
      <head>
        ...
        <style>
          ...
          .header h1 { font-size: ${headerSize}; margin: 0; text-transform: uppercase; font-weight: bold; }
          .info { margin-bottom: 10px; font-size: ${fontSize}; }
          .info p { margin: 2px 0; }
          ...
          .total-final { font-size: ${headerSize}; font-weight: bold; margin-top: 10px; border-top: 1px dashed #000; padding-top: 5px; }
