...
export default function LojaDashboard() {
  ...
  const checkAuth = async () => {
    ...
    if (!profileResponse.ok || !profileData.perfil) { router.push('/'); return }

    const perfil = profileData.perfil
    let lojaQuery = supabase.from('lojas').select('*')
    
    if (perfil.tipo === 'loja') lojaQuery = lojaQuery.eq('perfil_id', perfil.id)
    const { data: lojaData } = await lojaQuery.single()

    if (!lojaData && perfil.tipo === 'loja') { router.push('/'); return }
    ...
    if (lojaData) {
      ...
      const { count: pedidosHojeCount } = await supabase.from('pedidos').select('*', { count: 'exact', head: true }).eq('loja_id', lojaData.id).gte('created_at', todayISO)
      const { data: pedidosHoje } = await supabase.from('pedidos').select('total').eq('loja_id', lojaData.id).gte('created_at', todayISO).not('status', 'eq', 'cancelado')
      const receitaHoje = pedidosHoje?.reduce((sum, p) => sum + (p.total || 0), 0) || 0
      const { count: pendentesCount } = await supabase.from('pedidos').select('*', { count: 'exact', head: true }).eq('loja_id', lojaData.id).in('status', ['pendente', 'aceito', 'preparando', 'pronto'])
      const { count: produtosCount } = await supabase.from('produtos').select('*', { count: 'exact', head: true }).eq('loja_id', lojaData.id).eq('disponivel', true)

      setStats({ pedidosHoje: pedidosHojeCount || 0, receitaHoje: receitaHoje, pedidosPendentes: pendentesCount || 0, produtosAtivos: produtosCount || 0 })
    }
    setLoading(false)
  }

  if (loading) return <div className="min-h-screen bg-gray-900 flex items-center justify-center"><div className="animate-spin rounded-full h-12 w-12 border-4 border-primary border-t-transparent"></div></div>
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900">
      <div className="container mx-auto px-4 py-8">
        <div className="mb-8">
          ...
          <p className="text-gray-400">Bem-vindo de volta, {loja?.nome_loja || 'Loja'}!</p>
        </div>
