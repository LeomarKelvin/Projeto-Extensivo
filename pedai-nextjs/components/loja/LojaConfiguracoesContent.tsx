...
interface FormData {
  ...
  tempo_entrega_max: number | string;
  tempo_retirada_min: number | string;
  tempo_retirada_max: number | string;
...
export default function LojaConfiguracoesContent() {
  ...
  const notify = (msg: string, type: 'success' | 'error' = 'success') => {
    setToastMessage(type === 'error' ? msg : msg)
    setToastType(type)
    setShowToast(true)
  ...
  const [formData, setFormData] = useState<FormData>({
    ...
    cep: '', logradouro: '', numero: '', bairro: '', cidade: '', estado: '', complemento: '', referencia: '', ocultar_endereco: false, endereco: '',
  ...
  const formatarCNPJ_CPF = (v: string) => {
    ...
    if (nums.length <= 11) return nums.replace(/(\d{3})(\d)/, '$1.$2').replace(/(\d{3})(\d)/, '$1.$2').replace(/(\d{3})(\d{1,2})/, '$1-$2').slice(0, 14)
  ...
  const loadData = async () => {
    ...
    if (loja) {
      ...
      setFormData({
        ...
        taxa_entrega: loja.taxa_entrega ?? 5.00,
        pedido_minimo: loja.pedido_minimo ?? 0.00,
  ...
  // --- HANDLERS OTIMIZADOS (Bloqueio de Scroll e Negativos) ---
  const handleNumberChange = (e: React.ChangeEvent<HTMLInputElement>, field: keyof FormData) => {
    let val = parseFloat(e.target.value)
    if (isNaN(val) || val < 0) val = 0 // Impede negativos
    setFormData({ ...formData, [field]: val })
  }

  // Bloqueia a mudança de valor ao rolar o scroll do mouse
  const handleWheel = (e: React.WheelEvent<HTMLInputElement>) => {
    (e.target as HTMLInputElement).blur()
  }

  // ... (Demais Handlers mantidos iguais) ...
  ...
  const handleContactAdd = () => { if (novoContato) { setFormData({ ...formData, contatos_extras: [...formData.contatos_extras, formatarTelefone(novoContato)] }); setNovoContato('') } }
  const handleContactRemove = (i: number) => { setFormData({ ...formData, contatos_extras: formData.contatos_extras.filter((_, idx) => idx !== i) }) }
  ...
  const handleSave = async () => {
    ...
    try {
      if (activeTab === 'dono') {
         if (userId) {
           const { error } = await supabase.from('perfis').update({ nome_completo: donoData.nome_completo, cpf: donoData.cpf, telefone: donoData.telefone }).eq('user_id', userId)
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="flex gap-2 mb-6 border-b border-gray-700 pb-1 overflow-x-auto">
          {['dados', 'dono', 'entrega', 'pagamento', 'operacao', 'sistemas', 'seguranca'].map(tab => (
            <button key={tab} onClick={() => setActiveTab(tab as any)} className={`px-4 py-2 font-medium whitespace-nowrap capitalize transition-colors ${activeTab === tab ? 'text-primary border-b-2 border-primary' : 'text-gray-400 hover:text-white'}`}>
        ...
        <div className="bg-gray-800 rounded-xl p-6 border border-gray-700 shadow-lg">
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {formData.contatos_extras.map((c, i) => (
                        <span key={i} className="bg-gray-900 border border-gray-600 px-2 py-1 rounded text-xs flex items-center gap-2 text-gray-300">{c} <button onClick={() => handleContactRemove(i)} className="text-red-400 hover:text-white">×</button></span>
                      ))}
                    </div>
                  </div>
                </div>
              ...
              <div>
                ...
                <div className="grid md:grid-cols-4 gap-4">
                  ...
                  <div>
                    ...
                    <input value={formData.estado} onChange={e => setFormData({...formData, estado: e.target.value.toUpperCase().slice(0, 2)})} maxLength={2} className="w-full bg-gray-900 text-white p-3 rounded border border-gray-600 uppercase" placeholder="UF" />
                  </div>
          ...
          {/* ABA ENTREGA (ATUALIZADA COM ONWHEEL) */}
          {activeTab === 'entrega' && (
            <div className="space-y-8">
              {/* Modos de Entrega */}
              <div>
              ...
              {/* Tempos */}
              <div>
                <h3 className="text-lg font-bold text-white mb-4 border-b border-gray-700 pb-2">Estimativa de Tempo (Minutos)</h3>
                <div className="grid md:grid-cols-2 gap-8">
                  <div className="bg-gray-900 p-4 rounded-lg border border-gray-600">
                    <h4 className="text-gray-400 text-sm mb-3 uppercase font-bold">Delivery</h4>
                    <div className="flex gap-2 items-center">
                      <input type="number" min="0" onWheel={handleWheel} value={formData.tempo_entrega_min} onChange={e => handleNumberChange(e, 'tempo_entrega_min')} className="w-20 bg-gray-800 text-white p-2 rounded border border-gray-500 text-center" />
                      <span className="text-gray-500">a</span>
                      <input type="number" min="0" onWheel={handleWheel} value={formData.tempo_entrega_max} onChange={e => handleNumberChange(e, 'tempo_entrega_max')} className="w-20 bg-gray-800 text-white p-2 rounded border border-gray-500 text-center" />
                      <span className="text-white text-sm">min</span>
                    </div>
                  </div>
                  <div className="bg-gray-900 p-4 rounded-lg border border-gray-600">
                    <h4 className="text-gray-400 text-sm mb-3 uppercase font-bold">Retirada</h4>
                    <div className="flex gap-2 items-center">
                      <input type="number" min="0" onWheel={handleWheel} value={formData.tempo_retirada_min} onChange={e => handleNumberChange(e, 'tempo_retirada_min')} className="w-20 bg-gray-800 text-white p-2 rounded border border-gray-500 text-center" />
                      <span className="text-gray-500">a</span>
                      <input type="number" min="0" onWheel={handleWheel} value={formData.tempo_retirada_max} onChange={e => handleNumberChange(e, 'tempo_retirada_max')} className="w-20 bg-gray-800 text-white p-2 rounded border border-gray-500 text-center" />
                      <span className="text-white text-sm">min</span>
                    </div>
                  </div>
                </div>
              ...
              <div>
                 ...
                 <div className="grid md:grid-cols-2 gap-4">
                    <div><label className="text-gray-400 text-sm">Taxa de Entrega Padrão (R$)</label><input type="number" step="0.50" min="0" onWheel={handleWheel} value={formData.taxa_entrega} onChange={e => handleNumberChange(e, 'taxa_entrega')} className="w-full bg-gray-900 text-white p-3 rounded border border-gray-600" /></div>
                    <div><label className="text-gray-400 text-sm">Pedido Mínimo (R$)</label><input type="number" step="1.00" min="0" onWheel={handleWheel} value={formData.pedido_minimo} onChange={e => handleNumberChange(e, 'pedido_minimo')} className="w-full bg-gray-900 text-white p-3 rounded border border-gray-600" /></div>
                 </div>
                 {/* Lista de Bairros (Mantida) */}
                 <div className="mt-6">
                    ...
                    <div className="bg-gray-900 p-4 rounded-lg border border-gray-600 flex gap-2 mb-2">
                      ...
                      <input type="number" placeholder="Taxa" min="0" onWheel={handleWheel} value={novoBairro.taxa} onChange={e => setNovoBairro({...novoBairro, taxa: e.target.value})} className="w-20 bg-gray-800 text-white p-2 rounded border border-gray-600" />
