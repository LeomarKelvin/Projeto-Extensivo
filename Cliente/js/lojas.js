...
async function buscarLojas() {
    ...
    try {
        ...
        lojas.forEach(loja => {
            ...
            // O MOLDE HTML ABAIXO AGORA É BASEADO NO SEU DESIGN ORIGINAL
            containerLojas.innerHTML += `
                <div class="store-card bg-white rounded-xl shadow-md overflow-hidden transition-all duration-300 hover:shadow-lg">
                    <div class="relative">
                        ...
                        <span class="badge absolute top-3 right-3 bg-primary text-secondary px-2 py-1 rounded-full text-xs font-semibold">Destaque</span>
                    </div>
                    <div class="p-4">
                        <div class="flex items-start">
                            <div class="w-16 h-16 rounded-full border-4 border-white -mt-10 mr-3 z-10 relative flex-shrink-0">
                                 <img src="${loja.url_imagem || 'https://placehold.co/100'}" alt="Logo da ${loja.nome}" class="w-full h-full object-cover rounded-full">
                            </div>
                            <div class="flex-grow">
                                ...
                                <div class="flex items-center mt-1">
                                    ...
                                    <span class="text-sm text-gray-600 ml-1">${rating} (${reviewCount})</span>
                                </div>
                            </div>
                        </div>
                        <div class="mt-3">
                        ...
                        <div class="mt-4 flex justify-between items-center">
                            <span class="text-green-600 font-medium text-sm">${promoText}</span>
                            <button onclick="adicionarAoCarrinho('${loja.nome}', 15.00, '${loja.id}')" class="btn-primary px-4 py-2 rounded-lg text-sm font-medium">Adicionar Item Teste</button>
                        </div>
                    </div>
                </div>
            `;
